...
class RandomPaddingCenterCrop:
    ...
    def __call__(self, im, label=None):
        ...
        if img_height == crop_height and img_width == crop_width:
            ...
            pad_width = max(crop_width - img_width, 0)
            if pad_height > 0 or pad_width > 0:
                left_pad = math.floor(pad_width / 2)
                top_pad = math.floor(pad_height / 2)
                if not self.normal_center:
                    left_pad = np.random.randint(pad_width+1)
                    top_pad = np.random.randint(pad_height+1)
            ...
            if crop_height > 0 and crop_width > 0:
                h_off = math.floor((img_height - crop_height) / 2)
                w_off = math.floor((img_width - crop_width) / 2)
                if not self.normal_center:
                    h_off = np.random.randint(img_height - crop_height + 1)
                    w_off = np.random.randint(img_width - crop_width + 1)
...
class RandomRotation90:
    ...
    Args:
        prob: A probability of adjusting hue. Default: 0.3.
    """
