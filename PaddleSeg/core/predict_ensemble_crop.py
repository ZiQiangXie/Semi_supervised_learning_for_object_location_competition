...
def predictEnsembleThree(model,
    ...
    model_1.eval()
    utils.utils.load_entire_model(model_2, model_path_2)
    model_2.eval()
    ...
    with paddle.no_grad():
        for i, im_path in enumerate(img_lists[local_rank]):
            ...
            im_2, _ = transforms_2(im_origin)
            im = im[np.newaxis, ...]
            im = paddle.to_tensor(im)
            im_2 = im_2[np.newaxis, ...]
            im_2 = paddle.to_tensor(im_2)
            ...
            preds = []
            if aug_pred:
                pred_2 = infer_crop.aug_inference(
                    model_2,
                    im_2,
                    ori_shape=ori_shape,
                    transforms=transforms_2.transforms,
                    scales=scales,
                    flip_horizontal=flip_horizontal,
                    flip_vertical=flip_vertical,
                    is_slide=is_slide,
